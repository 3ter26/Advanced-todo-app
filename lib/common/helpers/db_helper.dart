...
class DbHelper {
  ...
  //We're done creating our tables, now we want to open The db + create tables.
  static Future<sql.Database> db() async {
    return sql.openDatabase(
      'dbestech', version: 1,
        // join(await sql.getDatabasesPath(), 'dbstech'),
  ...
  static Future<int> createItem(Task task) async {
    ...
    final id = await db.insert('todos', task.toJson(),
        conflictAlgorithm: sql.ConflictAlgorithm
            .replace); //if we have any conflicts, for e.g. in pur ID's -> so we need to replace
    return id;
  }
